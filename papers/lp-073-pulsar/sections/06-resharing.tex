...
The Pulsar and Corona signing protocols consume a per-party \texttt{KeyShare} struct that carries more than just the bare Shamir share:
\[
...
The Quasar epoch manager (\texttt{consensus/protocol/quasar/epoch.go}) currently regenerates a \emph{fresh} group public key at every validator-set rotation by calling \texttt{coronaThreshold.GenerateKeys}. Under \textsc{Reshare}, the group public key is invariant for the entire group lineage; the rotation method is renamed for clarity:
