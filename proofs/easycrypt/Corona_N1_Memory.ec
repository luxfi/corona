(* -------------------------------------------------------------------- *)
(* Corona -- byte-level memory model                                    *)
(* -------------------------------------------------------------------- *)
(* Mirror of Pulsar_N1_Memory.ec, decomplected from layout files.       *)
(*                                                                      *)
(* This file holds the byte-level memory abstraction shared by every    *)
(* Corona layout and refinement file. Three ideas live here, and only   *)
(* three:                                                               *)
(*                                                                      *)
...
(*                                                                      *)
(* These three pieces are independent of any Corona / R-LWE specific    *)
(* type or codec. The decomplect benefit: the Combine_Layout and        *)
(* Sign_Layout files share Memory + Signature_Codec only, never each    *)
(* other's encoders.                                                    *)
(*                                                                      *)
(* The memory model corresponds to the JModel_x86 `Glob.mem`            *)
(* storeW8 / loadW8 view at the byte level, abstracted from word-       *)
(* aligned semantics that aren't needed for the layout proofs.          *)
(* -------------------------------------------------------------------- *)
