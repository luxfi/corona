...
#   <luxcpp>/crypto/pulsar/test/kat/corona_oracle_v2/{prng_blake2_xof,
...
CORONA_DIR="${KAT_BASE}/test/kat/corona_oracle_v2"
...
mkdir -p "$(dirname "${RESHARE_OUT}")" "$(dirname "${DKG2_OUT}")" "${CORONA_DIR}"
...
echo "[4/5] corona_oracle_v2 emit --out ${CORONA_DIR}"
go run ./cmd/corona_oracle_v2 emit --out "${CORONA_DIR}" >/dev/null
