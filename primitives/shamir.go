...
// ShamirSecretSharingGeneralWithSeed is a deterministic variant of
// ShamirSecretSharingGeneral. Instead of drawing the (t-1) random
...
//
//	for polyIndex, poly := range s {
//	    for coeffIndex, secret := range poly.coeffs {
//	        draw a_1..a_{t-1}
//	        for partyIdx in 1..k { share_partyIdx[polyIndex][coeffIndex] = P(partyIdx) }
//	    }
//	}
func ShamirSecretSharingGeneralWithSeed(r *ring.Ring, s []ring.Poly, t, k int, seed []byte) map[int]structs.Vector[ring.Poly] {
	degree := r.N()
	q := r.Modulus()
